...
async function prepareIndex(response, request) {
  ...
  fixedHtml = fixedHtml.replace(/<script>[\s\S]*?installAppButton[\s\S]*?Die App kann momentan nicht installiert werden[\s\S]*?<\/script>/gi, "");
...
(function(){
  ...
  function renderRealNextMatch(){
    ...
    if (dateEl) dateEl.textContent = match.start.toLocaleDateString('de-DE', {weekday:'long',day:'2-digit',month:'2-digit',year:'numeric'}) + ' · ' + match.start.toLocaleTimeString('de-DE', {hour:'2-digit',minute:'2-digit'}) + ' Uhr';
    ...
    const d = box.querySelector('#days'); const h = box.querySelector('#hours'); const m = box.querySelector('#minutes'); const s = box.querySelector('#seconds');
...
window.addEventListener("DOMContentLoaded",function(){
  ...
  return new Response(fixedHtml, { status: response.status, statusText: response.statusText, headers });
}

self.addEventListener("fetch", event => {
  const request = event.request;
  if (request.mode === "navigate") {
    event.respondWith(fetch(request, { cache: "no-store" }).then(response => prepareIndex(response, request)));
    return;
  }
...
self.addEventListener("push", event => {
  ...
  catch { data = { title: "ATSV Forchheim", body: event.data ? event.data.text() : "Neue Nachricht" }; }
  event.waitUntil(self.registration.showNotification(data.title || "ATSV Forchheim", {
    ...
    data: { url: data.url || "./index.html", title: data.title || "ATSV Forchheim", body: data.body || "Es gibt eine neue Nachricht." },
    vibrate: [200, 100, 200]
  }));
});
