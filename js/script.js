// ATSV Fanseite – Push-Diagnose und Push-Aktivierung
// Dieses Skript nutzt das bereits vorhandene Supabase-Push-System.
...
async function atsVEnablePush() {
  ...
  try {
    ...
    if (error) {
      throw new Error(`Supabase ${error.code || "Fehler"}: ${error.message || "Unbekannter Fehler"}${error.details ? ` | Details: ${error.details}` : ""}${error.hint ? ` | Hinweis: ${error.hint}` : ""}`);
    }
...
// Auch der bisherige Inline-Aufruf enablePushNotifications() wird jetzt auf
// die neue, diagnostizierbare Funktion umgeleitet.
